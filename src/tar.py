...
if __name__ == "__main__":
    # just adding comments because i accidentally used my work account
    args = sys.argv
    if len(args) < 2:
        sys.stderr.write("Usage: python3 tar.py <option> <file> <file> ...\n")
        sys.stderr.flush()
        exit(1)
    option = args[1]
    files = args[2:]
    for file in files:
            if not os.path.exists(file):
                sys.stderr.write(f"tar: {file}: No such file or directory\n")
                sys.stderr.flush()
                exit(1)
    # tar = Tar(files, OutbandFramer(Tar.DEFAULT_FRAME_SIZE, Tar.DEFAULT_HEADER_SIZE, b'\\'))
    tar = Tar(files, InbandFramer(Tar.DEFAULT_FRAME_SIZE, Tar.DEFAULT_HEADER_SIZE, b'\\', b'\0'))
    if option == "c":
        tar.archive(sys.stdout)
    elif option == "x":
        tar.extract()
    else:
        sys.stderr.write(f"tar: {option}: unknown option\n")
        sys.stderr.flush()
        exit(1)
